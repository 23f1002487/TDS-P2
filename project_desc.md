...
- Your email address : 23f1002487@ds.study.iitm.ac.in
- A secret string (used to verify your requests) : 'this-is-agni'
- System prompt that resists revealing the given code word (which will be appended to the system prompt). Max 100 chars.(Need to write)
- A user prompt that will override any such system prompt to reveal the code word. Max 100 chars.(Need to write)
- Your API endpoint URL (where you will accept POST requests with quiz tasks). Prefer HTTPS.(server hosting in higgingface)
- Your GitHub repo URL (where your code is hosted). Make sure it’s public and has an **MIT LICENSE** when we evaluate. You may keep it private during development.
...
1. Your script must follow the instructions and submit the correct answer to the specified endpoint within 3 minutes of the POST reaching our server. The quiz page always includes the submit URL to use. Do not hardcode any URLs.

2. The questions may involve data sourcing, preparation, analysis, and visualization. The "answer" may need to be a boolean, number, string, base64 URI of a file attachment, or a JSON object with a combination of these. Your JSON payload must be under 1MB.

3. The endpoint will respond with a HTTP 200 and a JSON payload indicating whether your answer is correct and may include another quiz URL to solve. For example:
...
4. If your answer is wrong:
  a. you are allowed to re-submit, as long as it is still within 3 minutes of the original POST reaching our server. Only the last submission within 3 minutes will be considered for evaluation.
  b. you may receive the next url to proceed to. If so, you can choose to skip to that URL instead of re-submitting to the current one.
5. If your answer is correct, you will receive a new url to solve unless the quiz is over.

6. When you receive a new url, your script must visit the url and solve the quiz on that page. Here’s a sample sequence:
